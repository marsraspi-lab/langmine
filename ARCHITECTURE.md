...
| Radon CC E-grade elimination | 2026-06-13 | ✅ Pass | Refactored `export` (E) and `mine_video` (E) to B-grade via extraction: `_export_build_notes`, `_export_store_media`, `_export_setup_deck_and_model`, `_export_deduplicate_and_add`, `_build_mine_result`, `_enrich_transcript_error`, `_mine_and_log`, `_parse_mine_request`. CI gate `radon cc --min C src/` now passes — all functions B or better. |
| Dead-code audit | 2026-06-13 | ✅ Pass | Runtime trace of full test suite confirmed zero genuinely dead code. 29 functions missed by REST tracing fall into three explained categories: adapters replaced by fakes (hexagonal architecture), language processor called through abstract interface, factory/startup called during `create_app()`. Every route handler has a corresponding REST test. Adapter isolation verified — each adapter tested with mocked HTTP, mocked subprocess, or real bundled data (CC-CEDICT, SUBTLEX, HSK). |
