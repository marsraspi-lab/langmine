...
def get_video_info(video_id_or_url: str, user_agent: str = "") -> dict[str, str]:
    """Fetch video metadata (title, channel, duration) via yt-dlp.

    Uses --skip-download so no video/audio data is downloaded.
    Returns dict with 'title', 'channel', 'duration_sec' keys.
    ...
    try:
        ...
        return {"title": "", "channel": "", "duration_sec": "0"}
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            return {
                ...
                "duration_sec": str(info.get("duration", 0) or 0),
            }
    except Exception:
        pass
    return {"title": "", "channel": "", "duration_sec": "0"}
