...
<div class="transcript-view">
	<div class="transcript-toolbar">
		...
		<div class="toolbar-actions">
			...
			<button
				class="toolbar-btn"
				class:active={showAnnotation}
				onclick={() => (showAnnotation = !showAnnotation)}
				title="Toggle annotations (R)"
			>
				🎨 Annotate
			</button>
		</div>
	...
	{:else}
		<div class="sentence-list">
			{#each sentences as sentence, idx (sentence.id)}
				<div
					id="s-{idx}"
					...
					onkeydown={(e) => {
						if (e.key === 'Enter' || e.key === ' ') activeSentenceIdx = idx;
					}}
					role="button"
					tabindex="0"
				>
					...
					<div class="sentence-content">
						...
						<div class="sentence-chinese">
							{#if showAnnotation && sentence.annotation?.length}
								...
								{#each sentence.annotation as entry, aidx (aidx)}
									<ruby class="annotated-char">
										{entry.char}<rt style="color: {TONE_COLORS[entry.tone] || '#9E9E9E'}"
											>{entry.reading}</rt
										>
									</ruby>
								{/each}
							{:else if sentence.words?.length}
								{#each sentence.words as word, widx (widx)}
									<span
										class="word-token word-{word.status}"
										onclick={(e) => handleWordClick(word, widx, idx, e)}
										onkeydown={(e) => {
											if (e.key === 'Enter' || e.key === ' ') handleWordClick(word, widx, idx, e);
										}}
										role="button"
										tabindex="0"
									>
							...
							{#if sentence.has_audio}
								<button
									...
									title="Replay audio (S/Space)">▶</button
								>
							{/if}
						</div>
						...
						{#if sentence.unknown_word && sentence.frequency_badge}
							<span class="freq-tag"
								>{sentence.frequency_badge}
								{sentence.unknown_word} #{sentence.unknown_word_rank}</span
							>
						{/if}
	...
	{#if activeWord}
		<div
			...
			onkeydown={(e) => {
				if (e.key === 'Enter' || e.key === ' ' || e.key === 'Escape') closePopover();
			}}
			role="button"
			tabindex="0"
		...
		<div class="word-popover">
			...
			<div class="popover-meta">
				{#if activeWord.word.hsk_level}
					<span class="hsk-badge hsk-{activeWord.word.hsk_level}"
						>HSK{activeWord.word.hsk_level}</span
					>
				{/if}
			...
			<div class="popover-status-row">
				Status: <span class="status-badge-inline word-{activeWord.word.status}"
					>{activeWord.word.status}</span
				>
			</div>
			<div class="popover-actions">
				...
				<button
					...
					disabled={activeWord.word.status === 'ignored'}>🚫 Ignore</button
				>
				<button
					class="popover-btn"
				...
				{#if activeWord.word.status !== 'proper-name'}
					<button class="popover-btn" onclick={() => handleMarkProperName(activeWord.word.token)}
						>👤 Mark as proper name</button
					>
				{/if}
				{#if activeWord.word.status === 'proper-name'}
					<button class="popover-btn" onclick={() => handleDismissProperName(activeWord.word.token)}
						>❌ Not a proper name</button
					>
				{/if}
				<button class="popover-btn" onclick={() => (showImagePicker = true)}
					>🖼️ Search images</button
				>
				<button class="popover-btn" onclick={() => showInDictionary(activeWord.word.token)}
...
<style>
	...
	.status-badge-inline.word-known {
		background: rgba(78, 204, 163, 0.2);
	}
	.status-badge-inline.word-learning {
		background: rgba(255, 167, 38, 0.2);
	}
	.status-badge-inline.word-ignored {
		background: rgba(150, 150, 150, 0.2);
	}
	.status-badge-inline.word-unknown {
		background: rgba(233, 69, 96, 0.2);
	}
	.status-badge-inline.word-proper-name {
