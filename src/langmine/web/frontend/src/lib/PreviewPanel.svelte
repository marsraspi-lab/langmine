...
{#if data}
  <div class="preview-panel" class:collapsed>
    <button class="preview-toggle" onclick={() => collapsed = !collapsed}>
      ...
      {#if !collapsed}
        <span class="toggle-dismiss" onclick={(e) => { e.stopPropagation(); data = null; }} role="button" tabindex="0" onkeydown={(e) => { if (e.key === 'Enter') data = null; }}>✕</span>
      {/if}
    </button>
    ...
    {#if !collapsed}
      ...
      <div class="preview-transcript">
        {#each data.sentences as sentence, idx}
