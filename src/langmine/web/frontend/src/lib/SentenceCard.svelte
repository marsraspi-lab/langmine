...
<div class="sentence-card" transition:fly={{ y: 20, duration: 200 }}>
	<div class="card-header">
		<span class="chinese-text">
			{#if displayWords.length > 0}
				{#each displayWords as word, idx (word.token + idx)}
					<span
		...
		<div class="editable-field" class:saving>
			...
			{:else}
				<span
					...
					onkeydown={(e) => {
						if (e.key === 'Enter' || e.key === ' ') startEdit('reading');
					}}
					role="button"
					tabindex="0"
					title="Click to edit"
		...
		<div class="editable-field" class:saving>
			...
			{:else}
				<span
					...
					onkeydown={(e) => {
						if (e.key === 'Enter' || e.key === ' ') startSegmentedEdit();
					}}
					role="button"
					tabindex="0"
				...
				{#if word.status === 'unknown' || word.status === 'learning'}
					<span class="word-annotation word-annotation-{idx}">
						...
						{#if word.frequency_rank}
							<span class="freq-badge" title="Rank {word.frequency_rank}"
								>{freqBadge(word.frequency_rank)} {freqRank(word.frequency_rank)}</span
							>
						{/if}
					</span>
				{/if}
			...
			<div class="popover-actions">
				...
				{#if word.status !== 'proper-name'}
					<button
						...
						disabled={togglingWord === activeWordIdx}>👤 Mark as proper name</button
					>
				{/if}
				<button class="popover-btn" onclick={() => showInDictionary(word.token)}
	...
	<div class="card-actions">
		{#if showMerge}
			<button class="btn-merge" onclick={() => onmerge(sentence.id)}> ⬆️ Merge </button>
		{/if}
		<button class="btn-keep" onclick={() => onkeep(sentence.id)}> 🟢 Keep </button>
		{#if SHOW_DELETE_CONFIRM}
			<button class="btn-delete" onclick={doDelete}> ⚠️ Confirm delete </button>
