	...
	>
		...
		<label class="file-upload-label" class:has-file={transcriptFile !== null}>
			<span
				>{transcriptFile
					? `📄 ${transcriptFile.name}`
					: '📂 Transcript .srt / .vtt (optional)'}</span
			>
			<input
	...
	{#if app.videos.length > 0}
		<div class="export-section">
			...
			<label class="force-update-label">
				<input type="checkbox" bind:checked={forceUpdateModel} disabled={app.exporting} />
				⚡ Update card templates
			</label>
			<label class="force-update-label">
				<input type="checkbox" bind:checked={clozeMode} disabled={app.exporting} />
