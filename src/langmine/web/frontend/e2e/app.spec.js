...
test.describe('LangMine SPA', () => {
	...
	test('cloze deletion checkbox is visible and togglable', async () => {
		await main.goto();
		const clozeCheckbox = main.page.locator('.force-update-label', {
			hasText: 'Cloze deletion cards'
		});
	...
	test('export with cloze sends card_type=cloze to API', async () => {
		...
		const clozeLabel = main.page.locator('.force-update-label', {
			hasText: 'Cloze deletion cards'
		});
	...
	test('manual mark as proper name from reading mode popover', async () => {
		...
		await expect(reading.properNameWords.filter({ hasText: '皇帝' }).first()).toBeVisible({
			timeout: 5000
		});
	...
	test('dismiss proper name removes brackets in reading mode', async () => {
		...
		await expect(reading.properNameWords.filter({ hasText: '李世民' })).toHaveCount(0, {
			timeout: 5000
		});
